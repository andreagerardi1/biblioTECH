...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($old_password) || empty($new_password) || empty($confirm_password)) {
        ...
        $result = pg_prepare($db, "password_lettore",$query);
        $result = pg_execute($db, "password_lettore", array($email_lettore));
        ...
        if ($result && pg_num_rows($result) > 0) {
            ...
            if (!password_verify($old_password, $row['password'])) {
                ...
                if ($new_password !== $confirm_password) {
                    ...
                    $update_result = pg_prepare($db, "aggiorna_pw_lettore",$update_query);
                    $update_result = pg_execute($db, "aggiorna_pw_lettore", array($hashed_new_password, $email_lettore));
      ...
      <div class="card shadow-sm p-4" style="max-width: 500px; width: 100%;">
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
          ...
          <div class="d-grid gap-2">
            ...
            <a href="area_personale.php" class="btn btn-secondary btn-lg">Annulla</a>
          </div>
        </form>
      </div>
    </div>
